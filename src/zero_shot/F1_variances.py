"""
This file is to compute F1 variances
"""
import os
import glob
import pandas as pd
...
def get_new_df(df, glist):
    df1 = df[['F1', 'seed']].groupby(glist).agg(["mean", "std", "size"]).reset_index()
    print(df1)
    print('{:.1f}, {:.1f}, {:.1f}'.format(df1['F1']['std'][0], df1['F1']['std'][1], df1['F1']['std'][2]))
    print('{:.1f}\\std{{{:.1f}}}'.format(df['F1'].mean(), df['F1'].std()))
...
if __name__ == '__main__':
    ...
    for pathname in ['manual_24_*base_orig*', 'manual_24_*large_orig*']:
        # f_path_list = glob.glob(os.path.join(model_path, 'manual_24_*base_orig*'))
        f_path_list = glob.glob(os.path.join(model_path, pathname))
