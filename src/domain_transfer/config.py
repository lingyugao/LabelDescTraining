...
def get_args():
    parser = argparse.ArgumentParser(description='domain transfer')
    ...
    parser.add_argument('-no_sample', action='store_true', help='use full training data')
    parser.add_argument('-train_size', type=int, default=10, help='training data size')
    ...
    if args.train:
        if args.manual:
            model_path += 'manual_' + '_'.join([str(args.train_size), args.pattern_type, str(args.pattern_id),
                                                ...
                                                str(args.epochs), str(args.no_dev),
